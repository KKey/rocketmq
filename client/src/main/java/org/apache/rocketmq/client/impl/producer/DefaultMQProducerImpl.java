...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                //检查mq producer group 的配置，是否为空，是否超过255长度，是否包含除数字字母外的特殊字符，不能为DEFAULT_PRODUCER
                this.checkConfig();

                //如果group不等于CLIENT_INNER_PRODUCER， 且instanceName为默认或设置的“DEFAULT”则修改instanceName=当前进程号
                ...
                //将当前DefaultMQProducer，以group 做为key添加到MQClientInstance的producerTable缓存Map中
    ...
    private SendResult sendDefaultImpl(
        ...
        long endTimestamp;
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());//NOTE 查找并更新当前TOPIC的路由信息
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            boolean callTimeout = false;
            MessageQueue mq = null;
            Exception exception = null;
            SendResult sendResult = null;
            //重试次数（包括当前次），同步方式时重试次数，异步方式时不进行重试
            int timesTotal = communicationMode == CommunicationMode.SYNC ? 1 + this.defaultMQProducer.getRetryTimesWhenSendFailed() : 1;
            int times = 0;
            String[] brokersSent = new String[timesTotal];
            for (; times < timesTotal; times++) {//KKEY 控制请求次数，同步模式：当前请求1次+可失败重试次数，异步模式：只有当前请求一次
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                //KKEY 选择一个消息队列，队列选择策略，基于故障延迟容忍策略去选择消息队列
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);//从缓存路由Map中取
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic);//NOTE 缓存没有从NAME_SERVER获取路由信息
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        //从缓存获取当前队列所属的broker地址，如果没有则从NameServer获取
        ...
        if (brokerAddr != null) {
            ...
            try {
                //如果不是批量消息设置唯一ID，批量消息在构造消息的时候就set了
                ...
                //判断是不是要压缩，大小大于默认压缩大小则直接压缩，并设置压缩标志位，批量消息不支持压缩
                //压缩使用jdk的java.util.zip.Deflater
                ...
                if (this.hasSendMessageHook()) {
                    ...
                    this.executeSendMessageHookBefore(context);//执行消息发送前的hook增强逻辑
                }
                //KKEY 构造消息请求报文基础信息SendMessageRequestHeader对象
        ...
        //如果缓存和nameServer都没有当前队列的broker信息，那就抛个错吧
